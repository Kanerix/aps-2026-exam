...
long long minimizePipe(std::vector<std::unordered_map<std::size_t, int>> &graph,
    ...
    long long max_flow_with_broken = maxFlow(graph, s, t, threshold);
    graph[m_s][m_t] = 0;
    long long max_flow_without_broken = maxFlow(graph, s, t, threshold);
    return max_flow_with_broken - max_flow_without_broken;
}
