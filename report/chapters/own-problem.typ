...
The max flow, which can be pushed this graph is $26$.
...
The Python solutions implement the Edmonds-Karp variation of the algorithm, while the c++ solutions implement the capacity scaling variant.
...
Edges are traversed $E^2$ times, as each run of DFS consumes $E$ edges, and consumes one edge by modifying the residual graph.
...
=== Edge Case Inputs
The edge case inputs were generated by using the random input generator and then afterwards manually adjusted to satisfy the edge cases, which they test.
...
Lower upper limit were tested and seemed to suffice.
...
The upper limit was also compared with other max flow problems on Kattis, where it was observed that $10^3$ was within the same order of magnitude, which helped verified that this was an acceptable upper limit.
